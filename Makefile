...
MANIFEST_SRC_FILES=$(shell find $(HTML_SRC_DIR) -name '*-manifest.json' -not -path '*/templates/*')
...
MANIFEST_DEST_FILES=${subst $(HTML_SRC_DIR),$(HTML_DEST_DIR),$(MANIFEST_SRC_FILES)}
...
$(HTML_DEST_DIR)/%-manifest.json: $(HTML_SRC_DIR)/%-manifest.json
	@cp $< $@
...
default: $(CSS_DEST_FILE) $(JS_DEST_FILE) $(MANIFEST_DEST_FILES) $(HTML_DEST_FILES) $(ICONS_DEST_DIR) $(IMAGES_DEST_DIR) $(IMAGES_DEST_FILES) $(ASSETS_DEST_DIR) $(ASSETS_DEST_FILES) $(FAVICON_ICO) $(ICON_180) $(SYNTH_DEST_DIR) $(SYNTH_DEST_FILES) $(INDEX_DEST_FILE) $(ATTRIBUTION_DEST_FILE)
	@echo Done with $(ENV)
